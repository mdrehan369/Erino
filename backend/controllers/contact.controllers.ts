...
const updateContact = asyncHandler(async (req: Request, res: Response) => {
    ...
    const prevContact = await contactModel.findById(id).exec()
    if (!prevContact) throw new ApiError(404, 'Contact does not exists')

    if (body.phoneNumber != prevContact.phoneNumber || body.email != prevContact.email)
        throw new ApiError(400, 'Cannot update phone number and email')


    const updatedContact = await contactModel
...
const getContact = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!contact) throw new ApiError(404, 'No contact found')

    return res.status(200).json(new ApiResponse(200, contact, 'Fetched!'))
})

export { addContact, deleteContact, getAllContact, updateContact, getContact }
